...
{ 0x3E, 0x63, 0x07, 0x3E, 0x78, 0x63, 0x3E, 0x00}, // U+0053 (S)
...
void setPixel(uint8_t x, uint8_t y, uint8_t v) {
	...
	int yoffset = y * (panelWidth/8);
	uint8_t mask = ~( 1 << (x%8));

	for( int plane = 0; plane < targetSubframes; plane++) {

		uint8_t* ptr =  (uint8_t *) displaybuf + plane*subframeSizeInByte  + yoffset  + x/8;
		uint8_t pix = *ptr;
		if( v & (1<<plane)) {
			*ptr = pix | ~ mask;
		} else {
